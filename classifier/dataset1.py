...
class GestureDataset(torch.utils.data.Dataset):
    ...
    def __read_annotations(self, subset: int = None) -> pd.DataFrame:
        ...
        all_real_target=self.conf.dataset.targets
        all_target=os.listdir("/home/vuhl/hagrid/hagrid-sample-500k-384p/hagrid_500k")
        ...
        def convert(t):
            return ["no gesture" for j in range(len(t))]
        ...
        final_final=final_final.sample(frac=1)

        final_all.head(1000).to_csv("final_all.csv",index=False)
